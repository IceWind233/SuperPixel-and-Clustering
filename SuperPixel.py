...
class SLIC:
    ...
    def __find_lowest_gradient_position(self, x, y):
        ...
        self.clusterCenter.append(np.array([
                                    (x / self.image_width) * 255,
                                    (y / self.image_height) * 255,
                                    self.image_lab[y, x, 0],
                                    self.image_lab[y, x, 1],
                                    self.image_lab[y, x, 2]
                                    ]))
    ...
    def draw_Kmeans_image(self, label):
        Kmeans_image = self.image.copy()
        idx = 0
        for cluster in self.clusters:
            for p in cluster.pixels:
                ...
                y = p[1]
                for i in range(3):
                    Kmeans_image[x][y][i] = self.randColor[label[idx][0]][i] / 2 \
                                            + Kmeans_image[x][y][i] / 2
    ...
    def kMeans(self):
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, self.Knum, 1.0)
        flags = cv2.KMEANS_RANDOM_CENTERS
        ret, label, center = cv2.kmeans(np.float32(self.clusterCenter), 10, None, criteria, 10, flags)
    ...
    def process(self):
        ...
        for _ in trange(self.iter):
            self.clustering()
        self.clusterCenter = np.array(self.clusterCenter)
        clustering_time = time() - clustering_time
        print(f"Clustering {self.iter} times take: {round(clustering_time, 2)}s")
        ret, label, center = self.kMeans()
        kmeansImage = self.draw_Kmeans_image(label)
...
if __name__ == '__main__':
    ...
    slic = SLIC(image, 1000, 10, iter=10)
    slic.process()
